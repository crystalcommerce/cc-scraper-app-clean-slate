    ...
    class CcScraper {
        ...
        async downloadEncodedText()   {
            let currentLinkObject = this.currentLinkObject,
                ...
                fileName = `${toUrl("enc" + " " +Object.keys(currentLinkObject.productProps).reduce((a, b) => {
                    a += currentLinkObject.productProps[b] + " ";
                    return a;
                }, "") + ` __dt-${Date.now()}` + ` __total-${this.productObjects.length}`)}.txt`;
        ...
        async goToLink(productObject)    {

            let url = productObject.productUri,
                queryObject = queryStringToObject(url);
                
            queryObject["__cc-single-product-scraper"] = true;
            
            url = [url.split("?").shift(), objectToQueryString(queryObject)].join("?");

            window.open(url, "_blank");
        }

        async singleProductEvaluator(evaluatorObject)   {
            await moderator(this.productObjects, (slicedArr) => {


                let promises = await slicedArr.map(productObject => {
                    return async function() {

                        // open the url on a new tab;
                        await goToLink(productObject);

                        await waitForSelector(productObject.urlLoaded);


                        // mark


                        evaluatorObject.callback(productObject);


                        await waitForSelector(productObject.productDataScraped);
                    }
                });
