...
async function getAllProducts({ccUtilities, categoryLinkObjects})  {
    ...
    await moderator(categoryLinkObjects, async (slicedArr) => {
        ...
        let promises = slicedArr.map((categoryLinkObject, index) => {
            return async function() {
                ...
                console.table({...categoryLinkObject, linkObjectIndex : index});

                console.log({
                    linkObjectIndex : index,
                    productsPerCategory,
...
async function initializeScript(mainDirPath) {
    ...
    let productObjects = await getAllProducts({ccUtilities, categoryLinkObjects});
