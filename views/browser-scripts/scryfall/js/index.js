...
const {toCapitalize, getValidatedPropValues, zipData, getAllObjectKeys, toNormalString} = window.ccUtilities;
...
function getCardBasicProps(cardObject, frameEffects)   {
    ...
    let productName = function(){
            let name = getValidatedPropValues(cardObject, ["name"]);

            return [name, frameEffects].filter(item => item !== null || item !== "").join("");
        }(),
        name = getValidatedPropValues(cardObject, ["name"]),
        imageUris = function(){
        ...
        cardType = getValidatedPropValues(cardObject, ["type_line"]),
        powTgh = function(){
        ...
        cardText = function(){
            let oracle_text = getValidatedPropValues(cardObject, ["oracle_text"]),
...
function showDownloadZipButton(foundSet, productObjects, callback)    {
    ...
    downloadButtonContainer.addEventListener("click", async (e) => {
        e.preventDefault();
...
(async function(){
    ...
    aButton.addEventListener("click", async (e) => {
        ...
        try {
            let {productObjects, foundSet} = await scrapeData(url) ;
